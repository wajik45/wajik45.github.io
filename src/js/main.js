...
((_) => {
   ...
   const lahir = new Date("02/28/2005");
   const sekarang = Date.now();
   const ambilUmur = new Date().getFullYear(sekarang) - lahir.getFullYear();
...
class HandleAos {
   ...
   kondisiElement(
      ...
      this.elementInView(array ? el[index] : el) &&
         this.toggleClass(pecah ? el[index] : el, loop, add, ms);
   }
}
class ElementAos extends HandleAos {
   ...
   aosTentang() {
      const tentangTextHeader = document.querySelectorAll(
         ".tentang-text-header"
      );
   ...
   aosPortfolio() {
      const portfolioTextHeader = document.querySelectorAll(
